Y.use('node', 'squarespace-gallery-ng', function(Y) {
  ...
  function getDimensionsFromNode(node) {
    var dimensions = node.getAttribute('data-image-dimensions');

    if (typeof dimensions === 'string') {
      var widthHeight = dimensions.split('x').map(function(x) {return Number(x);});
      return { width: widthHeight[0], height: widthHeight[1] };
    }
  ...
  Y.on('domready', function() {
    ...
    if (Y.one('body').hasClass('collection-type-gallery')) {
      ...
      if (Y.one('.gallery-view-start-on-grid')) {
    ...
    function scrollToTop() {
      ...
      new Y.Anim({ node: document.scrollingElement || scrollNodes, to: { scroll: [0,0] }, duration: 0.9, easing: 'easeBoth' }).run();
    }
